...
picomatch$1.parse = (pattern, options) => {
  ...
  return parse_1(pattern, Object.assign({}, options, {
    fastpaths: false
  }));
};
/**
...
const micromatch = (list, patterns, options) => {
  ...
  for (let i = 0; i < patterns.length; i++) {
    let isMatch = picomatch(String(patterns[i]), Object.assign({}, options, {
      onResult
    }), true);
...
micromatch.capture = (glob, input, options) => {
  ...
  let regex = picomatch.makeRe(String(glob), Object.assign({}, options, {
    capture: true
  }));
...
function* expandPatterns$1(context) {
...
function* expandPatternsInternal(context) {
  ...
  for (const pattern of context.filePatterns) {
    ...
    if (stat) {
      if (stat.isFile()) {
        ...
        entries.push({
          type: "dir",
          glob: escapePathForGlob(fixWindowsSlashes$1(pattern)) + "/" + getSupportedFilesGlob(),
          input: pattern
        });
      }
    } else if (pattern[0] === "!") {
  ...
  for (const {
    ...
    try {
      result = out.sync(glob, globOptions);
    } catch ({
      message
    }) {
...
function cliifyOptions(object, apiDetailedOptionMap) {
  return fromPairs_1(Object.entries(object || {}).map(([key, value]) => {
...
function getOptionsForFile$1(context, filepath) {
...
function format$1(context, input, opt) {
  ...
  if (context.argv["debug-print-comments"]) {
    return {
      formatted: src.format(JSON.stringify(src.formatWithCursor(input, opt).comments || []), {
        parser: "json"
      })
    };
  }
...
function formatStdin$1(context) {
...
function formatFiles$1(context) {
...
function getContextOptions(plugins, pluginSearchDirs) {
  ...
  const detailedOptionMap = normalizeDetailedOptionMap$1(Object.assign({}, createDetailedOptionMap$1(supportOptions), constant.options));
...
function logResolvedConfigPathOrDie(context) {
  ...
  const configFile = src.resolveConfigFile.sync(file);
...
function main(rawArguments, logger) {
  ...
  if (context.argv["support-info"]) {
    logger.log(src.format(fastJsonStableStringify(src.getSupportInfo()), {
      parser: "json"
    }));
    return;
  }
